...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            #print("BartDecoderLayer: {}".format(encoder_attention_mask.shape))  # こいつをtensor([], size=(1, 1, 512, 0))→size=(1, 1, 512, 1024)にする
...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            raise ValueError("You cannot specify inputs_embeds for now")
        else:
        ...
        input_ids = torch.ones(len(input_ids_list), l, dtype=torch.int32, device=device)
        attention_mask = torch.zeros(len(attention_mask_list), l, dtype=torch.int32, device=device)
        for i, (_input_ids, _attention_mask) in enumerate(zip(input_ids_list, attention_mask_list)):
            input_ids[i, :len(_input_ids[0])] = _input_ids[0]
            attention_mask[i, :len(_attention_mask[0])] = _attention_mask[0]
        ...
        for i, (_hidden_states, _attention_mask) in enumerate(zip(hidden_states, original_attention_mask)):
            if i == 0:
                title_hidden_states = torch.mean(_hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device=device)), dim=0)
            else:
                abst_hidden_states = torch.mean(_hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device=device)), dim=0)
        ...
        for i, (_hidden_states, _attention_mask) in enumerate(zip(hidden_states, original_attention_mask)):
            if i == 0:
                return_hidden_states = _hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device=device))
            else:
                return_hidden_states = torch.cat((return_hidden_states, matching_list[i-1] * _hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device=device))), 0)

        if not return_dict:
            return tuple(v for v in [hidden_states, encoder_states, all_attentions] if v is not None)
        return BaseModelOutput(
            last_hidden_state=return_hidden_states.unsqueeze(0), hidden_states=encoder_states, attentions=all_attentions
        )
